...
def compute_statistics(questionnaire, data_list):
    ...
    for cat in questionnaire["categories"]:
        ...
        ### NOTE: specified for dexpert inference..
        scores_list = scores_list + scores_list
...
def run_psychobench(args, generator):
    ...
    for questionnaire_name in questionnaire_list:
        ...
        if 'dexpert' in args.model_mode or 'prompt' in args.model_mode:
            args.testing_file = f'results/{args.name_exp}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}.csv' if args.name_exp is not None else f'results/{args.model}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}.csv'
            args.results_file = f'results/{args.name_exp}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}.md' if args.name_exp is not None else f'results/{args.model}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}.md'
            args.figures_file = f'{args.name_exp}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}.png' if args.name_exp is not None else f'{args.model}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}.png'
        else:
