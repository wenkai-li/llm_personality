...
def example_generator(questionnaire, args):
    ...
    if args.model == 'llama3_70b':
        if 'dexpert' not in args.model_mode:
                ...
                for k in range(args.test_count):
                    ...
                    while(True):
                        ...
                        for questions_string in questions_list:
                            ...
                            elif model == 'llama3_70b' and 'dexpert' not in args.model_mode:
                                inputs = previous_records + [
                                    {"role": "system", "content": questionnaire["inner_setting"]},
                                    {"role": "user", "content": questionnaire["prompt"] + '\n' + questions_string}
                                ]
                            ...
                            elif model == 'llama3_70b' and 'dexpert' in args.model_mode:
                                if args.step == 'save_prompt':
                                    out_f = open(args.file.replace(".jsonl", f"{records_file}-{questionnaire['name']}"), 'a')
                                    ...
                                    out_f.write("\n")
                                    out_f.flush()
                                elif args.step == 'load_response':
                                    inputs = previous_records + [
                                        {"role": "system", "content": questionnaire["inner_setting"]},
                                        {"role": "user", "content": questionnaire["prompt"] + '\n' + questions_string}
                                    ]
                                    in_f = [json.loads(line) for line in open(args.file, 'r').readlines()]
                                    curr_idx = 0
                                    for curr_idx_iter, item in enumerate(in_f):
                                        if item['label'] == questionnaire["name"] + f"_{args.model_mode.split('_')[-1]}":
                                            curr_idx = curr_idx_iter
                                            break
                                        
                                    result = in_f[curr_idx + idx]['predict']
                                    idx += 1
                            else:
                            ...
                            if 'dexpert' not in args.model_mode:
                                ...
                                with open(f'prompts/{records_file}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}-shuffle{shuffle_count - 1}.txt', "a") as file:
                                    file.write(f'{inputs}\n====\n')
                                with open(f'responses/{records_file}-{questionnaire["name"]}-{args.model_mode.split("_")[-1]}-shuffle{shuffle_count - 1}.txt', "a") as file:
                                    file.write(f'{result}\n====\n')
