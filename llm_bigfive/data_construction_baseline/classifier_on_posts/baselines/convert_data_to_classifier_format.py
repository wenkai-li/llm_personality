...
if __name__ == "__main__":
    ...
    df['agr_z_label'] = 2
    df['neu_z_label'] = 2
    df.rename(inplace=True, columns={"gpt4omini": "message"})
    df.loc[(df['trait'] == 'o') & (df['level'] == 'high'), 'ope_z_label'] = 0
    df.loc[(df['trait'] == 'o') & (df['level'] == 'low'), 'ope_z_label'] = 1
    df.loc[(df['trait'] == 'c') & (df['level'] == 'high'), 'con_z_label'] = 0
    df.loc[(df['trait'] == 'c') & (df['level'] == 'low'), 'con_z_label'] = 1
    ...
    for trait in ['o', 'c', 'e', 'a', 'n']:
        tmp_df = df.loc[df['trait'] == trait]
        tmp_df.to_csv(f"/home/jiaruil5/personality/llm_personality/llm_bigfive/classifier/results_baselines_new_baseline/generator_predictions_{trait}.csv")
