...
def main(args):
    ...
    if args.model_name_or_path:
        ...
        if args.use_vllm:
            ...
            if args.use_lora:
                generations = model.generate(prompts, sampling_params, lora_request=LoRARequest("dpo_adapter", 1, args.lora_path))
            else:
                generations = model.generate(prompts, sampling_params)
            # print("The Generations are: {}".format(generations))
            prompt_to_output = {
    ...
    } for example, output, pred, ground_true in zip(test_data, outputs, predictions, targets)]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_lora",
        default=False,
        help="If true, we will use lora when create LLM"
    )
    parser.add_argument(
        "--lora_path",
        default=None,
