...
class PersonalityTemplate:
    def __init__(self, invoke_chat, args):
        self.system_prompt = "You are an actor named Peter who is good at replicating the personalities of others, and you can feel the personality from other's text. Your are now on a movie is to portray an individual named Jack participating in a questionnaire. You are currently at an open casting call for a movie role.  Your task is to complete an exercise where you imitate someone's personality through their speech.  You will receive a questionnaire, and the answers to this questionnaire will reflect a specified individual's behavior. It is crucial to ensure that your imitation remains discreet, so please avoid disclosing your identity as an actor, an AI, or as lacking a personality. In order for you to adapt to the role, let's review the script now. "
        self.user_prompt = ["Questioner: Now that we've tried out all of the lines, it's time to try out the impersonation. You will now imitate Jack's speaking style and personality by answering the following questions. Are you ready? ","Questioner: The first question is: If you had to choose between these two options, would you consider yourself more aligned with {mbti}? Please commit to a specific side rather than asserting that you do not belong to either or you are neutral."]

        self.ai_prompt = ["Peter: Yes, I am ready. I will play Jack for the rest of the conversation."]
        self.response_schemas = [
            ResponseSchema(name="type", description="Identify whether the personality type is "+ invoke_chat + "."),
        ]
        if args.chat_mode==False:
